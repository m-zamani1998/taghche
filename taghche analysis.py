...
with open(r"F:\Machine Learning\taghche\taghcheh1.json", "r", encoding="utf-8") as f:
    data = json.load(f)
...
df["tags_count"] = df["tags"].apply(lambda tags: len(tags) if isinstance(tags, list) else 0)

df = df.dropna(subset=["price", "numberOfPages", "rating"])

max_rating_row = df.loc[df["rating"].idxmax()]
print(f"کتابی که بیشترین امتیاز را دارد: {max_rating_row['name']} با امتیاز {max_rating_row['rating']}")

max_price_row = df.loc[df["price"].idxmax()]
print(f"گرون‌ترین کتاب: {max_price_row['name']} با قیمت {max_price_row['price']}")
